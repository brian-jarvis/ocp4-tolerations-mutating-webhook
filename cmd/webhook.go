...
const (
  admissionWebhookAnnotationInjectKey = "mutator-webhook.bry/mutate"
  admissionWebhookAnnotationStatusKey = "mutator-webhook.bry/status"
)
...
type WhSvrParameters struct {
  ...
  cfgFile        string // path to sidecar injector configuration file
}
...
// (https://github.com/kubernetes/kubernetes/issues/57982)
func applyDefaultsWorkaround(tolerations []corev1.Toleration) {
...
func addToleration(target, added []corev1.Toleration, basePath string) (patch []patchOperation) {
  ...
  for _, add := range added {
    ...
    if first {
      ...
      // if tolerations already exist in the target remove it
      for _, t := range target {
        indx := 0
        glog.Infof("checking for %v in target %v", add.Key, t.Key)
        if t.Key == add.Key {
          patch = append(patch, patchOperation{
            Op:    "remove",
            Path:  basePath + "/" + strconv.Itoa(indx),
          })
        }
        indx++
      }
...
func (whsvr *WebhookServer) mutate(ar *v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
  ...
  applyDefaultsWorkaround(whsvr.mutateConfig.Tolerations)
